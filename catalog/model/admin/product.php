...
class ModelAdminProduct extends Model {
	...
	public function setSortOrder($prts, $store_id){
		...
		$i = 0;
		foreach ($prts as $prt) {
			$this->db->query("UPDATE " . DB_PREFIX . "product SET sort_order = '".($i++)."' WHERE product_id = '".(int)$prt."' AND store_id = '".(int)$store_id."'");
		}

	}

	public function setStatus($product_ids, $status, $checkStore = false){
		if(count($product_ids) < 1) return;
		$ids = join(',', $product_ids);
		$sql = "UPDATE " . DB_PREFIX . "product SET status = '".(int)$status."' WHERE product_id IN (".$this->db->escape($ids).")";
		if($checkStore) $sql .= " AND store_id = '".$this->config->get('config_store_id')."'";
		$this->db->query($sql);
	}

	public function addProduct($data) {

		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET store_id = ".(int)$data['store_id'].", model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . (int)$data['tax_class_id'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(), date_modified = NOW()");
	...
	public function deleteProduct($product_id, $checkOwner = false) {
		if($checkOwner){
			$query = $this->db->query("SELECT store_id FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'");
			if($query->num_rows < 1) return;
			if(intval($query->row['store_id']) != intval($this->config->get('config_store_id'))) return;
		}
	...
	public function getProduct($product_id) {
		...
		$data = &$query->row;
		if($data){
			$data['d'] = $this->getProductDescriptions($product_id);
		}

		return $data;
	}

	public function setProduct($product_id, $data){
		$sql = "UPDATE " . DB_PREFIX . "product SET ";
		$first = true;
	...
	public function getProductBasics($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'subcat_id'        => $query->row['subcat_id'],
				'discount_amt'     => $query->row['discount_amt'],
				'discount_type'    => $query->row['discount_type'],
				'gst'    		   => $query->row['gst'],
				'hsn'    		   => $query->row['hsn']
			);
		} else {
	...
	public function getProductsByIds($ids){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON(p.product_id = pd.product_id) WHERE p.product_id IN (".$ids.") AND pd.language_id = '".(int)$this->config->get('config_language_id')."' GROUP BY p.product_id");
		return $query->rows;
	}
	...
	public function setProductCategories($product_id, $cats){
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "'");
	...
	public function setProductImages($product_id, $to_keep, $to_add){
		...
		$this->db->query($sql);
		if($to_add){
			foreach ($to_add as $img) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_image SET product_id = '" . (int)$product_id . "', image = '" . $this->db->escape($img) . "', sort_order = '0' ");
			}
		}
	}
	...
	public function setProductDiscount($product_id, $discount){
		$discounts = $this->getProductDiscounts($product_id);
		if(count($discounts)){
			$this->db->query("UPDATE " . DB_PREFIX . "product_discount SET price = '" . (float)$discount . "' WHERE product_id = '" . (int)$product_id . "'");
		}else{
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_discount SET product_id = '" . (int)$product_id . "', customer_group_id = '1', quantity = '1', priority = '0', price = '" . (float)$discount . "', date_start = '0000-00-00', date_end = '0000-00-00'");
		}
	}
