...
class ModelAdminOrder extends Model {
	...
	public function markAsPaid($order_id){
		$sql = "UPDATE `" . DB_PREFIX . "order` SET paid = 1 WHERE order_id = '".(int)$order_id."'";
		$this->db->query($sql);
	}

	public function getOrderBasis($order_id){
		$sql = "SELECT order_id, total, payment_method, del_timing FROM `" . DB_PREFIX . "order` WHERE order_id = '".(int)$order_id."'";
		$query = $this->db->query($sql);
		return $query->row;
	}

	public function getNewestOrders($time){
		$sql = "SELECT total FROM `" . DB_PREFIX . "order` WHERE order_status_id = 1 AND UNIX_TIMESTAMP(date_modified) > ".(int)$time." AND store_id = ".(int)$this->config->get('config_store_id');
		$query = $this->db->query($sql);
		return $query->rows;
	}
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'paid'           => intval($order_query->row['paid']) == 1,
				'can_cancel' => $this->canCancel($order_id)
			);
		} else {
			return;
		}
	...
	public function getOrders($data = array()) {
		...
		if ($data['limited']) {
			$sql .= " AND o.date_added > NOW() - INTERVAL 1 DAY";
		}
		...
		if (isset($data['store'])) {
			$sql .= " AND o.store_id = '" . (int)$data['store'] . "'";
		}
	...
	public function removeProduct($order_id, $product_id, $checkStore = true){
		if($checkStore){
			$query = $this->db->query("SELECT store_id FROM " . DB_PREFIX . "order WHERE order_id = '".(int)$order_id."'");
			if((int)$query->row['store_id'] != (int)$this->config->get('config_store_id')){
				return;
			}
		}
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '".(int)$order_id."' AND product_id = '" . (int)$product_id . "' LIMIT 0, 1");
		if($query->num_rows){
			$item = $query->row;
			$quantity = intval($item['quantity']);
			$ltotal = floatval($item['price']) * $quantity;
			$this->db->query("DELETE FROM ".DB_PREFIX."order_product WHERE order_product_id = '".$item['order_product_id']."'");
			$this->db->query("UPDATE ".DB_PREFIX."order SET total = total - ".$ltotal." WHERE order_id = '".(int)$order_id."'");
			$this->db->query("UPDATE ".DB_PREFIX."product SET quantity = quantity + ".$quantity." WHERE product_id = '".(int)$product_id."'");
		}
	}
	...
	public function getNewOrdersBasisByStoreId($store_id, $lastCheck, $limited = false) {
		$sql = "SELECT order_id, total FROM `" . DB_PREFIX . "order` WHERE store_id = '" . (int)$store_id . "' AND UNIX_TIMESTAMP(date_added) > '".(int)$lastCheck."'";
		if($limited){
			$sql .= " AND UNIX_TIMESTAMP(date_added) > '".(time() - 3600 * 24)."'";
		}
		$query = $this->db->query($sql);
