...
class ControllerCheckoutShippingAddress extends Controller {
	...
	public function save() {
		...
		$this->request->post['zone_id'] = 1492;
		...
		if(isset($this->request->post['del_date'])){
			$this->session->data['del_date'] = $this->request->post['del_date'];
		}else{
			$this->session->data['del_timing'] = '';
		}
		$this->load->model('admin/store');
		$bsd = $this->model_admin_store->getStoreBSD($this->config->get('config_store_id'));

		$this->session->data['del_timing'] = 'From ' . $bsd['timing_from'] . ' to ' . $bsd['timing_to'];
		...
		if (!$json) {
			...
			if (utf8_strlen($this->request->post['del_date']) != 10) {
				$json['error']['warning'] = 'Please select delivery date';
			}
